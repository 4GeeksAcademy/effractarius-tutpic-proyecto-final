...
const CrearReceta = () => {
    ...
    const submitReceta = async () => {
        try {
            ...
            var response = await fetch(`${backendUrl}recetas/crear`, {
                headers: {
            ...
            if (!response.ok) {
                throw new Error("fallo en la creacion de receta")
            }

            let data = await response.json()
            console.log("data:",data)

            crearTags(data.receta.id)
    ...
    const crearTags = async (id) =>{
        try {
            var payload = {
                receta_id:id,
                ...tags
            }
            var response = await fetch(`${backendUrl}tags/crear`, {
                headers: {
                    "Content-Type": "application/json",
                    "Authorization": `Bearer ${token}`
                },
            ...
            if(!response.ok){
                throw new Error("error creacion de tags")
            }
